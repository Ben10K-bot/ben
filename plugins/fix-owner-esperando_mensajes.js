...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '*[❗] مجلد MyBen 10ssion غير موجود أو فارغ.*'}, {quoted: m});
    }
    ...
    if (filesDeleted === 0) {
      await conn.sendMessage(m.chat, {text: '*[❗] لم يتم العثور على أي ملف لحذفه في مجلد MyBen 10ssion.*'}, {quoted: m});
    } else {
