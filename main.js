...
global.authFile = `MyBen 10Session`;
...
const connectionOptions = {
  ...
  browser: ['Sasuke&Ben 10', 'Safari', '1.0.0'],
  version,
...
let prekey = []
let directorio = readdirSync("./MyBen 10Session")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MyBen 10Session/${files}`)
})
} 
...
const directories = ['./MyBen 10Session/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `🤖 مده النشاط: ${uptime} ┃ 👑 من صنع ساسكي و بن تن ┃ 🔗 حساب المطورين: https://heylink.me/MoXg/`;
